...
export function useSwipeBack(onBack: () => void, enabled?: Ref<boolean>) {
  ...
  /**
   * 檢查觸控目標是否在可水平滾動/拖動/滑動的容器內
   * 這些容器的交互不應觸發右滑返回
   */
  function isInScrollableOrDraggable(target: EventTarget | null): boolean {
    if (!target || !(target instanceof Element)) return false;
    ...
    while (el) {
      if (el instanceof HTMLElement) {
        ...
        // 元素可水平滾動且內容確實超出容器
        if (canScrollH && el.scrollWidth > el.clientWidth) return true;
      }
      el = el.parentElement;
    }
  ...
  function onTouchStart(e: TouchEvent) {
    ...
    if (isInScrollableOrDraggable(e.target)) return;
