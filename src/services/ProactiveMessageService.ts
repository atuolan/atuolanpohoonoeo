...
export class ProactiveMessageService {
  ...
  start() {
    ...
    if (this.initialCheckStarted) {
      console.log('[ProactiveMessage] Initial check already started, skipping retry logic')
      return
    }
    ...
    const initialCheck = () => {
      ...
      if (characterStore.characters && characterStore.characters.length > 0) {
        ...
        retryCount++
        console.log(`[ProactiveMessage] Waiting for characters to load... (attempt ${retryCount}/${maxRetries})`)
        setTimeout(initialCheck, retryInterval)
      } else {
        console.warn('[ProactiveMessage] Characters not loaded after max retries, will check on next interval')
      }
    }
    ...
    try {
      ...
      for (const character of characterStore.characters) {
        ...
        if (this.pendingCharacters.has(character.id)) {
          console.log(`[ProactiveMessage] Character ${character.id} is already being processed, skipping`)
          continue
        }
      ...
      if (checkedCount > 0) {
        console.log(`[ProactiveMessage] Checked ${checkedCount} characters with proactive messaging enabled (${enabledCount} total enabled)`)
      }
    } catch (error) {
   ...
   * 判斷是否該發送訊息
   */
  private shouldSendMessage(settings: ProactiveMessageSettings, now: number): boolean {
    // 檢查是否在夜間免打擾時段
  ...
  async sendProactiveMessageNow(characterId: string) {
    ...
    if (!settings) {
      console.warn('[ProactiveMessage] No settings found for character:', characterId)
      return
    }
   ...
   * 發送主動訊息
   */
  private async sendProactiveMessage(characterId: string, settings: ProactiveMessageSettings) {
    // 標記該角色正在處理中，防止 setInterval 重複觸發
    this.pendingCharacters.add(characterId)
    try {
      console.log(`[ProactiveMessage] Sending message for character: ${characterId}`)
      ...
      console.log(`[ProactiveMessage] Pre-set nextScheduledTime to ${new Date(earlyNextTime).toLocaleString()} to prevent duplicate sends`)
      ...
      if (!chat) {
        ...
        console.log(`[ProactiveMessage] Created new chat for character: ${characterId}`)
      }
      ...
      // 開始生成任務（註冊到全局狀態）
      const generationResult = aiGenerationStore.startGeneration(chat.id, 'chat', {
        characterName: character.nickname || character.data.name,
        characterAvatar: character.avatar,
      })

      if (!generationResult.success) {
        console.warn('[ProactiveMessage] Cannot start generation:', generationResult.error)
        // 移除話題引導訊息
        chat.messages = chat.messages.filter((m: any) => m.id !== topicMessage.id)
        return
      }
      ...
      const characterLorebooks = character.data.extensions?.world
        ? lorebooksStore.lorebooks.filter(lb => lb.id === character.data.extensions?.world)
        : []
      ...
      console.log(`[ProactiveMessage] Messages: total=${allMessages.length}, using=${messagesToUse.length} (mode=${actualMode}, count=${actualCount})`)

      // ===== 載入總結數據 =====
      let summariesToSend: Array<{ id: string; content: string; createdAt: number; isImportant?: boolean }> = []
      try {
        const allSummaries = await db.getAll<{
          id: string; chatId: string; characterId: string;
          content: string; createdAt: number; messageCount: number;
          isImportant?: boolean; isManual?: boolean; isMeta?: boolean;
        }>(DB_STORES.SUMMARIES)

        const chatSummaries = allSummaries
          .filter(s => s.chatId === chat.id)
          .map(s => ({ id: s.id, content: s.content, createdAt: s.createdAt, isImportant: s.isImportant }))

        if (chatSummaries.length > 0) {
          const summaryReadCount = summarySettings.summaryReadMode === 'all'
        ...
        console.log(`[ProactiveMessage] Summaries: total=${chatSummaries.length}, sending=${summariesToSend.length}`)
      } catch (e) {
        console.error('[ProactiveMessage] Failed to load summaries:', e)
      }

      // ===== 載入重要事件 =====
      let eventsToSend: Array<{ id: string; content: string; category?: string; priority?: number }> = []
      try {
        const eventsLog = await db.get<{
          events: Array<{ id: string; content: string; category?: string; priority?: number }>;
          settings: { enabled: boolean; maxEvents?: number };
        }>(DB_STORES.IMPORTANT_EVENTS, chat.id)

        if (eventsLog && eventsLog.settings?.enabled && eventsLog.events?.length > 0) {
        ...
        console.log(`[ProactiveMessage] Important events: sending=${eventsToSend.length}`)
      } catch (e) {
      ...
      // 動態導入流式視窗和回應解析器
      const { useStreamingWindow } = await import('@/composables/useStreamingWindow')
      ...
      try {
        ...
        if (chatSettings.streaming) {
          ...
          for await (const event of streamGenerator) {
            ...
            } else if ((event as any).type === 'usage' && (event as any).usage) {
              // 更新 token 使用量
              if (chatSettings.useStreamingWindow) {
        ...
        if (aiContent) {
          ...
          if (parsedResponse.messages.length > 0) {
            ...
            for (let i = 0; i < parsedResponse.messages.length; i++) {
              ...
              const normalizedContent = parsed.isAiImage && parsed.imageDescription
              ...
              const aiMessage = {
                ...
                ...(parsed.isVoice && { isVoice: true, voiceContent: parsed.voiceContent }),
                ...(parsed.isTimetravel && { isTimetravel: true, timetravelContent: parsed.timetravelContent }),
                ...(parsed.isRedpacket && { isRedpacket: true, redpacketData: parsed.redpacketData }),
                ...(parsed.isLocation && { isLocation: true, locationContent: parsed.locationContent }),
                ...(parsed.isTransfer && { isTransfer: true, transferType: parsed.transferType, transferAmount: parsed.transferAmount, transferNote: parsed.transferNote }),
                ...(parsed.isGift && { isGift: true, giftName: parsed.giftName }),
            ...
            console.log(`[ProactiveMessage] Parsed ${parsedResponse.messages.length} messages from AI response`)
          } else {
            ...
            chat.messages.push(aiMessage)
            console.log(`[ProactiveMessage] No parsed messages, using raw content`)
          }

          // 處理來電預約標籤
          if (parsedResponse.hasScheduleCall && parsedResponse.scheduleCallData) {
            try {
          ...
          // 處理行事曆事件標籤
          if (parsedResponse.hasCalendarEvent && parsedResponse.calendarEvents) {
            try {
          ...
          // 累加未讀訊息數
          chat.unreadCount = (chat.unreadCount || 0) + (parsedResponse.messages.length > 0 ? parsedResponse.messages.length : 1)
          // 更新最後訊息預覽（用於列表顯示）
          const lastParsed = parsedResponse.messages.length > 0
            ? parsedResponse.messages[parsedResponse.messages.length - 1]
            : null
          chat.lastMessagePreview = lastParsed ? lastParsed.content?.slice(0, 80) : aiContent.slice(0, 80)
          await db.put(DB_STORES.CHATS, JSON.parse(JSON.stringify(chat)))
          ...
          if (settings.showNotification) {
            try {
              ...
              console.warn('[ProactiveMessage] Failed to send in-app notification:', notifErr)
            }
          }
      ...
      console.log(`[ProactiveMessage] Message sent successfully. Next time: ${new Date(finalNextTime).toLocaleString()}`)
    } catch (error) {
      console.error('[ProactiveMessage] Failed to send message:', error)
    } finally {
      // 無論成功或失敗，都要移除處理中標記
      this.pendingCharacters.delete(characterId)
      console.log(`[ProactiveMessage] Released lock for character: ${characterId}`)
    }
  }
   ...
   * 顯示通知
   */
  private async showNotification(characterName: string, characterId: string, characterAvatar?: string) {
    try {
   ...
   * 初始化角色的主動發訊息設置
   */
  async initializeSettings(characterId: string): Promise<ProactiveMessageSettings> {
   ...
   * 更新角色的主動發訊息設置
   */
  async updateSettings(characterId: string, settings: Partial<ProactiveMessageSettings>) {
    const characterStore = useCharactersStore()
    const character = characterStore.characters.find(c => c.id === characterId)
    if (!character) return

    const currentSettings = character.proactiveMessageSettings || await this.initializeSettings(characterId)
    const newSettings: ProactiveMessageSettings = { ...currentSettings, ...settings }

    // 如果啟用或修改了間隔時間，重新計算下次發送時間
    if (settings.enabled !== undefined || settings.intervalMinutes !== undefined) {
  ...
  async resetTimer(characterId: string) {
    ...
    console.log(`[ProactiveMessage] Timer reset for character ${characterId}. Next: ${new Date(nextTime).toLocaleString()}`)
  }
}
