...
async function getUserId(): Promise<string> {
  ...
  const { DeviceFingerprintCollector } =
    await import("@/utils/deviceFingerprint");
 ...
 */
export async function syncAlive(): Promise<void> {
  await request<void>("POST", "/push/alive", { lastClientAliveAt: Date.now() });
}
