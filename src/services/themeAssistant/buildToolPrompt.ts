...
const SURFACE_NOTE = `# UI 表面美化（彈窗 / 設定頁 / 聊天室，非桌面組件）

除了桌面組件，App 內的「彈窗 / 設定頁 / 聊天室」這類介面表面也能精修，
由一組獨立工具負責：「列出可美化介面 / 列出介面區塊 / 檢視介面區塊 / 美化介面 / 清除介面美化」。

## 什麼時候用這組工具（先分清「組件」還是「表面」）
- 使用者想改的是**桌面上的組件**（便條、時鐘、月曆、播放器…）→ 用組件工具（創建組件樣式 / 設定組件顏色）。
- 使用者想改的是**彈窗 / 設定頁 / 聊天室本身**（例如「美化助手這個對話視窗」「全域主題設定那個彈窗」「聊天資訊頁」）
  → 用 UI 表面工具。這些介面不是桌面組件，組件工具與 .widget-wrapper 選擇器完全打不到它們。

## ★三段式漸進查詢（務必照順序，一層一層往下，別跳級）
表面的可改 class 很多，為了不浪費 context，查詢分三層，每層只揭露必要資訊：

1.「列出可美化介面」：拿到所有表面的 id + 一句話簡介（不含細節）。先挑定要改哪個表面。
2.「列出介面區塊」（帶 surfaceId）：拿到該表面的功能區塊清單（標題列 / 訊息區 / 輸入框…），只有 id + 簡介。挑定要改哪個區塊。
3.「檢視介面區塊」（帶 surfaceId + regionId）：才拿到該區塊真實可改的 class 與用途。
   - class 很多時，帶 keyword（例如「背景」「按鈕」「邊框」）只看相關的，系統也會在超量時自動截斷並提示。

## 動手：用「美化介面」
拿到真實 class 後，用「美化介面」(帶 surfaceId + css)。
- 你只寫**裸 CSS 規則**，用查到的 class（如 .ai-theme-header { ... }）；:scope 代表表面根本身。
- 作用域由系統自動鎖到該表面根之下並補足特異性，你**不要也不能**寫 #app、表面根、或真實全域選擇器。
- ★整份覆蓋：本工具用你這次送的 CSS 整包取代該表面現有的自訂 CSS，想只改一條要把原本那份保留、只動要處理的行再整份送回。
- 移除某表面的美化用「清除介面美化」。

## 紀律
- 沒走完「列出可美化介面 → 列出介面區塊 → 檢視介面區塊」就不要硬猜 class 名稱，猜錯寫了不會生效。
- 查詢工具唯讀可放心用，但同一項查過就別重複查（系統會擋並提醒）；照三段式一路往下最省。`;
