...
export const useOracleStore = defineStore('oracle', () => {
  ...
  function drawCardsByInput(cardNumbers: number[]): {
    ...
    // 驗證牌號範圍（1-52）
    const invalidNumbers = cardNumbers.filter((n) => n < 1 || n > 52)
    if (invalidNumbers.length > 0) {
      return {
        success: false,
        error: `牌號必須在 1-52 之間，無效的牌號：${invalidNumbers.join(', ')}`,
      }
    }
    ...
    // 如果在 shuffle 階段，先執行洗牌（直接洗牌不等待動畫）
    ...
    const selectedCards: OracleDrawnCard[] = cardNumbers.map((num, idx) => {
