...
聊天 API 現在支援 SillyTavern 風格提示詞後處理與角色工具呼叫。功能只在一對一聊天啟用，群聊工具呼叫者驗證不包含在本階段。

## 設定

- **提示詞後處理**：`none`、`claude`、`merge`、`merge_tools`、`semi`、`semi_tools`、`strict`、`strict_tools`、`single`。
- **工具協議**：`auto` 優先原生 OpenAI/Claude 工具；供應商只在 HTTP `400`/`422` 且明確表示不支援 `tools` 或 `tool_calls` 時，最多切換一次文字協議。`native` 不切換，`text` 強制文字協議，`disabled` 關閉工具。
- **全域工具開關**：關閉後不會送出工具定義，也不會執行模型提出的工具。
- **角色權限**：角色編輯頁可切換整體權限、分類與單一工具。單一工具覆寫優先於分類，未知工具一律拒絕。

低風險工具會在生成迴圈中自動執行；建立行程與排程來電等高風險工具會先顯示確認視窗。停止生成、切換聊天或離開頁面時，待確認操作會失效。

第一批工具包括時間、天氣、行事曆、音樂、桌布、記憶與來電排程。工具參數會經 JSON Schema 驗證；每輪最多 5 次往返、每個工具 15 秒逾時、結果最多 8,000 字元，且不接受任意 JavaScript、URL、檔案路徑或 IndexedDB 寫入。
