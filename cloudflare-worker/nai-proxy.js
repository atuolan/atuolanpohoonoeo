...
function withCors(response, origin) {
  ...
  h.set(
    ...
    'server,cf-ray,cf-cache-status,via,x-upstream-status,x-upstream-host',
  );
 ...
 * 這些 headers 可能導致 NovelAI 辨識出請求來自代理並拒絕服務 (503)
 */
function buildUpstreamHeaders(request, targetHost, bodyLength) {
  ...
  // 偽裝 Origin 和 Referer，讓上游認為是正常的瀏覽器請求
  // 注意：NovelAI 圖片 API host 是 image.novelai.net（仍屬 novelai.net 網域）
  ...
  // iOS WebView/部分上游：過度偽造 sec-ch-ua / sec-fetch 反而可能觸發風控
  // 這裡保守處理：不主動注入 Sec-CH-UA/Sec-Fetch 系列，交由 Cloudflare 自然轉發/或直接不帶。
  // 如需更激進偽裝，建議做成可配置開關，而不是硬編碼。
    ...
    try {
      ...
      if (attempt < maxRetries) {
        ...
        if (response.status === 429) {
          waitMs = 10000 * (attempt + 1); // 429: 10s, 20s, 30s
        } else if (response.status === 503) {
          waitMs = 8000 * (attempt + 1); // 503: 8s, 16s, 24s（專門針對 503 優化）
        } else {
          waitMs = 5000 * (attempt + 1); // 其他 5xx: 5s, 10s, 15s
        }
...
async function handleRequest(request) {
    ...
    // 建立乾淨的上游 headers（過濾掉 Cloudflare 注入的 headers）
    ...
    try {
      ...
      const upstreamResponse = await fetchUpstreamWithRetry(
        upstream,
        {
          ...
          body: body,
          // 禁用 Cloudflare 的自動快取和壓縮行為，避免干擾上游
          ...(isNAI && {
            cf: {
              cacheTtl: 0,
      ...
      // 診斷用：讓前端可區分 503 來源（Worker 自己回/上游回）
      ...
      // 如果是 503 且已重試失敗，加入診斷資訊
      if (upstreamResponse.status === 503) {
        const body503 = await upstreamResponse.text().catch(() => '');
        return new Response(JSON.stringify({
          error: 'NovelAI 服務暫時不可用 (503)',
          message: '可能原因：1) NovelAI 服務器負載過高 2) 網絡連接不穩定 3) API Key 使用頻率過高',
          suggestion: '建議：稍後重試（已自動重試 3 次）或切換網絡環境',
          upstream_status: 503,
...
// ── 導出（兼容 ES Module 和 Service Worker 格式）──────────────

// ES Module 格式（Cloudflare Workers 新版）
export default {
  fetch: handleRequest,
};

// Service Worker 格式（Cloudflare Workers 舊版）
