  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt < maxRetries) {
        ...
        console.log(
          `[Retry] Status ${response.status}, waiting ${waitMs}ms before retry ${attempt + 1}/${maxRetries}`,
        );
      ...
      if (attempt < maxRetries) {
        ...
        console.log(
          `[Retry] Network error, waiting ${waitMs}ms before retry ${attempt + 1}/${maxRetries}`,
        );
        await new Promise((r) => setTimeout(r, waitMs));
        continue;
      }
